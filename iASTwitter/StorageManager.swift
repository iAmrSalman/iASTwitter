...
//  TrueMe
//
//  Created by Amr Salman on 7/20/17.
...
class StorageManager {
  ...
  func store(data: Data, jsonType: JSONType, fileName: String) throws {
    ...
    case .array:
      do {
        guard let array = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.mutableContainers) as? [Any] else { throw iASError.invalid(fileName, data) }
        try write(jsonText: try getJSONText(fromArray: array), toUrl: try getUrl(forKey: fileName))
      } catch {
        throw error
      }
    case .dictionary:
      do {
        guard let dictionary = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions.mutableContainers) as? [String: Any] else { throw iASError.invalid(fileName, data) }
        try write(jsonText: try getJSONText(fromDictionary: dictionary), toUrl: try getUrl(forKey: fileName))
      } catch {
        throw error
      }
